...
template<>
Task<char*>::Task(char* t1, char* t2,int t3){
    strcpy(time_buf,t1);
    strcpy(time_buf,t2);
    length = t3;
}
 ...
 */
template<>
char* Task<char*>::GetValue(){
    return value;
}
template<>
long Task<char*>::operator+(Task a)
{
    return (atol(value)+atol(a.value));
}
long operator-(Task<char*> a,Task<char*> b)
{
    return (atol(a.value)-atol(b.value));
}
long operator++(Task<char*> a)
{
    return (atol(a.value)+1);
}
long operator++(Task<char*> a,int)
{
    Task<char*> oldValue(a.value);
    a.value++;
    return atol(oldValue.value);
}
template<>
long Task<char*>::operator=(Task<char*> a){
    if(a.value == value)
        return atol(value);
    value = strdup(a.value);
    return atol(value);
}
ostream& operator<<(ostream &stream,Task<char*> &t){
    try
    {
        stream << t.value;
    }
 ...
 */
template<>
istream& operator>>(istream &stream,Task<char*> &t){
    try
    {
